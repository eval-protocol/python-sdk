...
class TestRolloutErrorHandling:
    ...
    async def test_agent_rollout_processor_429_error(self):
        ...
        config = RolloutProcessorConfig(
            completion_params={"model": "gpt-4"}, mcp_config_path="", logger=default_logger  # Empty to avoid MCP setup
        )
        ...
        with patch("eval_protocol.pytest.default_agent_rollout_processor.LiteLLMPolicy") as mock_policy_class:
            ...
            result = []
            async for row in default_agent_rollout_processor([test_row], config):
                result.append(row)

            assert len(result) == 1
            assert result[0].rollout_status.status == "error"
            assert result[0].rollout_status.termination_reason is not None
            assert (
                "429" in result[0].rollout_status.termination_reason
                or "rate limit" in result[0].rollout_status.termination_reason.lower()
            )
    ...
    async def test_agent_rollout_processor_bad_request_error(self):
        ...
        config = RolloutProcessorConfig(
            completion_params={"model": "gpt-4"}, mcp_config_path="", logger=default_logger
        )
        ...
        with patch("eval_protocol.pytest.default_agent_rollout_processor.LiteLLMPolicy") as mock_policy_class:
            ...
            assert result[0].rollout_status.termination_reason is not None
            assert (
                "content" in result[0].rollout_status.termination_reason
                or "BadRequest" in result[0].rollout_status.termination_reason
            )
    ...
    async def test_single_turn_rollout_processor_429_error(self):
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            result = []
            async for row in default_single_turn_rollout_processor([test_row], config):
                result.append(row)
    ...
    async def test_multiple_rows_with_mixed_errors(self):
        ...
        with patch("importlib.import_module") as mock_import:
            ...
            result = []
            async for row in default_single_turn_rollout_processor([row1, row2], config):
                result.append(row)
            ...
            for row in result:
                ...
                assert row.rollout_status.termination_reason is not None
                assert (
                    "429" in row.rollout_status.termination_reason
                    or "rate limit" in row.rollout_status.termination_reason.lower()
                )
    ...
    def test_rollout_status_initialization(self):
        ...
        assert status.status == "running"  # Default from the model
        assert status.termination_reason == ""  # Default empty string

        # Test explicit initialization
        status = RolloutStatus(status="error", termination_reason="Test error")
        assert status.status == "error"
        assert status.termination_reason == "Test error"
