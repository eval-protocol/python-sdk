...
def create_mock_row(row_id: str = "test-row") -> EvaluationRow:
    return EvaluationRow(
        input_metadata=InputMetadata(row_id=row_id, completion_params={"model": "test-model"}),
...
@pytest.mark.asyncio
async def test_scheduler_basic_execution(mock_logger, mock_eval_executor, base_config):
...
@pytest.mark.asyncio
async def test_concurrency_control(mock_logger, mock_eval_executor, base_config):
    """
    ...
    with patch("eval_protocol.pytest.priority_scheduler.rollout_processor_with_retry", side_effect=mock_rollout_gen):
        ...
        assert max_active_rollouts_seen <= max_rollouts, (
            f"Rollout concurrency exceeded: {max_active_rollouts_seen} > {max_rollouts}"
        )
...
@pytest.mark.asyncio
async def test_priority_scheduling(mock_logger, mock_eval_executor, base_config):
    """
    ...
    with patch("eval_protocol.pytest.priority_scheduler.rollout_processor_with_retry", side_effect=mock_rollout_gen):
        ...
        expected = ["row-0_run_0", "row-0_run_1", "row-1_run_0", "row-1_run_1"]

        assert execution_order == expected


@pytest.mark.asyncio
async def test_worker_scaling(mock_logger, mock_eval_executor, base_config):
    """
    ...
    class InstrumentedScheduler(PriorityRolloutScheduler):
        ...
        async def schedule_dataset(self, *args):
            ...
            for i in range(expected_workers):
                task = RolloutTask(
                    priority=(1, i), row=dataset[0], run_indices=[], config=base_config, row_index=0, history=[]
                )
...
@pytest.mark.asyncio
async def test_groupwise_mode(mock_logger, mock_eval_executor, base_config):
    """
