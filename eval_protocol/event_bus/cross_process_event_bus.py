...
class CrossProcessEventBus(EventBus):
    """
    Cross-process event bus implementation using the configured storage backend.
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        if db_path is None:
            ...
            from eval_protocol.event_bus import _get_default_db_filename

            eval_protocol_dir = find_eval_protocol_dir()
            db_path = os.path.join(eval_protocol_dir, _get_default_db_filename())

        from eval_protocol.event_bus import get_event_bus_database

        self._db: EventBusDatabase = get_event_bus_database(db_path)
        self._running = False
