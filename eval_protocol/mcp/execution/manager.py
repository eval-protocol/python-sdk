...
class ExecutionManager:
    ...
    async def _execute_rollout(
        ...
        @retry(
            ...
            wait=wait_exponential(multiplier=5, min=random.randint(10, 30), max=120),
            retry=retry_if_exception_type(RetryableError),
        )
        async def _inner_rollout() -> str:
            need_retry = False
            nonlocal failure_reason
            try:
                logging.info(f"inner_rollout start rollout {session.dataset_row.id} {session.session_id}")
                current_observation, tool_schema = await envs.reset(session)
                system_prompt = dataset_row.system_prompt
                logging.info(f"inner_rollout finish env reset{session.dataset_row.id} {session.session_id}")
                            ...
                            if user_simulator is None:
                                ...
                                if recording_mode:
                                    policy.log_conversation_state_for_playback(
                                        rollout_idx, step - 1, conversation_history
                                    )
        ...
        try:
            await _inner_rollout()
            # if retry successfully, reset failure reason
            failure_reason = None
        except:
            logger.error(f"{session.dataset_row.id} {session.session_id} failed after retry")
            pass
