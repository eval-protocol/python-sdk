...
from eval_protocol.mcp_agent.config import AppConfig, BackendServerConfig, RemoteApiConfig
...
class RemoteHttpOrchestrationClient(AbstractOrchestrationClient):
    ...
    async def startup(self) -> None:
        ...
        timeout_config = httpx.Timeout(
            ...
            connect=self.app_config.global_remote_api_defaults.get(
                "connect_timeout", 5.0
            ),
        )
    ...
    async def _make_request(
        ...
        try:
            logger.debug(
                f"Making {method} request to {url} with payload: {json_payload} and params: {params}"
            )
            ...
            raise RuntimeError(
                f"Remote API request failed: Network error calling {url}"
            ) from e
    ...
    async def provision_instances(
        ...
        response = await self._make_request(
            "POST", create_url, remote_api_config, json_payload=payload
        )
        ...
        for i, inst_data in enumerate(response_data):
            ...
            if not remote_instance_id or not mcp_endpoint_url:
                ...
                raise ValueError(
                    f"Remote API response for instance creation is incomplete: {inst_data}"
                )
        ...
        if (
            len(provisioned_instances_info) != num_instances
            and num_instances > 0
        ...
        for instance in instances:
            ...
            backend_cfg = next(
                (
                    ...
                    if b.backend_name_ref == instance.backend_name_ref
                ),
                None,
            )
            if not backend_cfg:
            ...
            remote_instance_id = instance.internal_instance_details.get(
                "remote_instance_id"
            )
            ...
            logger.info(
                f"Requesting deprovision of remote instance {remote_instance_id} via {delete_url}"
            )
            try:
                await self._make_request("DELETE", delete_url, remote_api_config)
                logger.info(
                    f"Successfully requested deprovision for remote instance {remote_instance_id}."
                )
            except Exception as e:
                # Log error but continue trying to deprovision other instances
                logger.error(
                    f"Failed to deprovision remote instance {remote_instance_id}: {e}"
                )
    ...
    async def call_tool_on_instance(
        ...
        if not backend_cfg:
            raise RuntimeError(
                f"Could not find BackendServerConfig for {instance.backend_name_ref}"
            )

        remote_api_config = self.app_config.get_remote_api_config(backend_cfg)
        if not remote_api_config:
            raise RuntimeError(
                f"RemoteApiConfig not found for backend {instance.backend_name_ref}."
            )
        ...
        if remote_api_config.call_tool_endpoint_template:
            ...
            logger.debug(
                f"Proxying tool {tool_name} to {target_url} for instance {instance.instance_id}"
            )
        else:
            # Call tool directly on the instance's MCP endpoint
            target_url = instance.mcp_endpoint_url
            logger.debug(
                f"Calling tool {tool_name} directly on {target_url} for instance {instance.instance_id}"
            )
    ...
    async def list_tools_on_instance(
        ...
        if instance.orchestration_mode != "remote_http_api":
            raise ValueError(
                "RemoteHttpOrchestrationClient can only list tools for 'remote_http_api' instances."
            )
