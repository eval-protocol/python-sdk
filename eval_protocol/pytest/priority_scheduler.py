...
class PriorityRolloutScheduler:
    ...
    def __init__(
        ...
        self.groups_buffer: Dict[int, List[EvaluationRow]] = defaultdict(
            list
        )  # buffer for group results. only flush to output buffer when a whole group is ready
    ...
    async def _process_task(self, task: RolloutTask):
        ...
        async def _run_eval(rows_to_eval: Union[EvaluationRow, List[EvaluationRow]]):
            ...
            rollout_id = (
                rows_to_eval[0].execution_metadata.rollout_id
                if isinstance(rows_to_eval, list)
                else rows_to_eval.execution_metadata.rollout_id
            )
            experiment_id = (
                rows_to_eval[0].execution_metadata.experiment_id
                if isinstance(rows_to_eval, list)
                else rows_to_eval.execution_metadata.experiment_id
            )
            run_id = (
                rows_to_eval[0].execution_metadata.run_id
                if isinstance(rows_to_eval, list)
                else rows_to_eval.execution_metadata.run_id
            )
            eval_res = None
        ...
        if current_batch_rows:
            for idx, row in current_batch_rows:
                async for result_row in rollout_processor_with_retry(self.rollout_processor, [row], task.config, idx):
    ...
    def _post_process_result(self, res: EvaluationRow):
        ...
        if res.eval_metadata is not None:
            if res.rollout_status.is_error():
                res.eval_metadata.status = Status.error(res.rollout_status.message, res.rollout_status.details)
            elif not (res.eval_metadata.status and res.eval_metadata.status.code != Status.Code.RUNNING):
