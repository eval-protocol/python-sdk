  ...
  return (
    <div className="border border-gray-200 rounded">
      <div className="flex items-center gap-3 px-3 py-1.5 bg-gray-50 border-b border-gray-200 text-xs">
        ...
        <span className="text-gray-500">{token_ids.length} tokens total</span>
        <span
      ...
      {showIds && (
        <div className="px-3 py-2 border-b border-gray-100 overflow-x-auto">
          <div className="text-[10px] text-gray-500 mb-1 font-semibold uppercase tracking-wider">
            Token IDs (gray=masked/prompt, colored=unmasked/completion by turn) — hover for text
            &amp; logprob
          </div>
  ...
  return (
    <div className="border border-gray-200 rounded">
      <div className="flex items-center gap-3 px-3 py-1.5 bg-gray-50 border-b border-gray-200 text-xs">
        ...
        {trace.tool_call_parser && <span className="text-gray-400">parser: {trace.tool_call_parser}</span>}
      </div>
          ...
          <div className="flex flex-wrap gap-0.5">
            {allIds.map((id, i) => (
              <span
                ...
                style={{
                  backgroundColor: i < promptLen ? "rgba(209, 213, 219, 0.4)" : turnColor(trace.step_index),
                }}
              >
                {id}
              </span>
      ...
      <div className="px-3 py-2 overflow-x-auto">
        <div className="whitespace-pre-wrap break-all leading-relaxed">
          {(detokens.length > 0 ? detokens : allIds.map((id) => `[${id}]`)).map((tok, i) => {
            const isPrompt = i < promptLen;
            const lpIdx = i - promptLen;
            const lp = !isPrompt && lpIdx >= 0 && lpIdx < logprobs.length ? logprobs[lpIdx] : null;
            return (
              <EpisodeToken
                key={i}
                token={tok}
...
function LogprobLegend() {
  ...
  return (
    <div className="flex items-center gap-1.5 text-[10px] text-gray-500">
      ...
      <div className="rounded" style={{ background: gradient, width: "80px", height: "12px" }} />
        ...
        {segments.map((seg, i) => (
          <span
            ...
            style={{
              ...
              borderBottom:
                seg.turnIdx > 0 ? `2px solid ${turnColor(seg.turnIdx).replace("0.5)", "0.9)")}` : "none",
            }}
          >
            {seg.text}
          </span>
...
export const TokenDebugView = ({ extra }: TokenDebugViewProps) => {
  ...
  if (!fullEpisode && tokenTurnTraces.length === 0) {
    return <div className="text-xs text-gray-400 italic p-2">No token data available</div>;
  }
    ...
    <div className="w-[700px] flex-shrink-0">
      ...
      <div className="max-h-[600px] overflow-y-auto space-y-3">
        {viewLevel === "text" && fullEpisode ? (
          ...
          <FullEpisodeView episode={fullEpisode} colorMode={colorMode} showIds={showIds} />
        ) : tokenTurnTraces.length > 0 ? (
          tokenTurnTraces.map((trace, i) => (
            <TurnSection key={i} trace={trace} colorMode={colorMode} showIds={showIds} />
          ))
        ) : (
          <div className="text-xs text-gray-400 italic p-2">No token data available for this view</div>
        )}
      </div>
    </div>
  );
