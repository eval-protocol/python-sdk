...
async def test_production_server_record_and_replay(
    ...
    assert len(evaluation_rows) == len(cliff_walking_dataset), "Should have evaluation row for each dataset entry"
...
async def test_cliff_walking_step_by_step(conda_isolation_recording_file):
    ...
    try:
        ...
        print(
            f"📊 Conda Isolation Evaluation: {eval_row.get_steps()} steps, reward: {eval_row.get_total_reward():.2f}, terminated: {eval_row.get_terminated()}"
        )
        # Actions are no longer available in EvaluationRow (they're embedded in messages)
        print(f"    Messages: {len(eval_row.messages)} total")
...
async def test_multi_environment_sessions(multi_env_dataset, multi_env_recording_file):
    ...
    try:
        ...
        start_time = time.time()
        evaluation_rows = await ep.rollout(envs, policy=policy, steps=14)
        duration = time.time() - start_time
...
async def test_fireworks_multi_environment_sessions(multi_env_dataset, fireworks_multi_env_recording_file):
    ...
    if is_ci and os.path.exists(fireworks_multi_env_recording_file):
        ...
        # Run playback
        start_time = time.time()
        playback_evaluation_rows = await ep.rollout(playback_envs, policy=playback_policy, steps=16)
        playback_duration = time.time() - start_time
    ...
    try:
        ...
        start_time = time.time()
        evaluation_rows = await ep.rollout(envs, policy=policy, steps=16)
        duration = time.time() - start_time
...
async def test_control_plane_state_querying(multi_env_dataset):
    ...
    try:
        ...
        for i, eval_row in enumerate(evaluation_rows):
            print(
                f"  Evaluation {i}: {eval_row.get_steps()} steps, reward: {eval_row.get_total_reward():.2f}, terminated: {eval_row.get_terminated()}"
            )
