...
@pytest.mark.asyncio
async def test_production_server_record_and_replay(production_server, lunar_lander_dataset, production_recording_file):
    ...
    if is_ci and os.path.exists(production_recording_file):
        ...
        playback_evaluation_rows = await ep.rollout(playback_envs, policy=playback_policy, steps=12)
    ...
    # Record evaluation rows (LunarLander may need more steps than simpler environments)
    start_time = time.time()
    ...
    assert len(evaluation_rows) == len(lunar_lander_dataset), "Should have evaluation row for each dataset entry"
    ...
    # Run playback
    start_time = time.time()
    playback_evaluation_rows = await ep.rollout(playback_envs, policy=playback_policy, steps=20)
    playback_duration = time.time() - start_time
...
async def test_multi_environment_sessions(multi_env_dataset, multi_env_recording_file):
    ...
    try:
        ...
        start_time = time.time()
        evaluation_rows = await ep.rollout(envs, policy=policy, steps=100)
        duration = time.time() - start_time
        ...
        for i, eval_row in enumerate(evaluation_rows):
            ...
            print(
                f"  Evaluation {i} (seed: {seed}, gravity: {gravity}): {eval_row.get_steps()} steps, reward: {eval_row.get_total_reward():.2f}, terminated: {eval_row.get_terminated()}, termination: {eval_row.get_termination_reason()}"
            )
    ...
    for env_idx in recorded_env_indices:
        ...
        if len(states) >= 2:
            if states[0] == states[1]:
                print(
                    f"    ⚠️  Env {env_idx}: Identical states between first two steps - may indicate session state issues"
                )
            else:
...
async def test_static_policy_functionality():
    ...
    policy = create_lunar_lander_static_policy(action_sequence=["FIRE_MAIN", "FIRE_LEFT", "FIRE_RIGHT", "NOTHING"])
