...
This guide demonstrates how to deploy a simple reward function to Google Cloud Run using the `eval-protocol` CLI. The example uses a basic `hello_world_reward` function found in `examples/gcp_cloud_run_deployment_example/dummy_rewards.py`.

## Overview

Deploying a reward function to GCP Cloud Run allows you to host it as a scalable, serverless HTTP endpoint. The `eval-protocol deploy` command automates much of this process, including containerization and service configuration.
...
*   `rewardkit.example.yaml`: An example configuration file for `eval-protocol`. This shows the structure for `rewardkit.yaml` if you choose to use one for GCP settings.
...
5.  **`eval-protocol` installed**: Ensure `eval-protocol` is installed in your Python environment (e.g., `pip install eval-protocol`).

## Setup

### `rewardkit.yaml` Configuration (Optional but Recommended)

The `eval-protocol` CLI can pick up GCP settings from a `rewardkit.yaml` file located in the directory from which you run the `eval-protocol deploy` command.

1.  **Create `rewardkit.yaml`**:
    You can copy the `examples/gcp_cloud_run_deployment_example/rewardkit.example.yaml` to the directory where you intend to run `eval-protocol deploy` (this could be the example directory itself, or your project root). Rename it to `rewardkit.yaml`.
    ```bash
...
2.  **Customize `rewardkit.yaml`**:
    ...
    evaluator_endpoint_keys: {} # Managed by eval-protocol for API key auth
    ```

**Note**: If you choose not to use a `rewardkit.yaml` file, you **must** provide all necessary GCP parameters (like `--gcp-project YOUR_PROJECT_ID`, `--gcp-region YOUR_REGION`) directly in the `eval-protocol deploy` command.
...
*   `--gcp-auth-mode api-key`: Configures the Cloud Run service with API key authentication. `eval-protocol` will generate a key, store it in GCP Secret Manager, and configure the service. The key is also saved to your local `rewardkit.yaml` under `evaluator_endpoint_keys`. This is the default if not specified.
...
1.  Create an Artifact Registry repository (default: `eval-protocol-evaluators`, or as specified in `rewardkit.yaml`).
