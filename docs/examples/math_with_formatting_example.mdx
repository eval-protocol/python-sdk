...
This guide explains how to evaluate models on math word problems using the `eval-protocol run` command, focusing on both the accuracy of the numerical answer and the adherence to a specific response format (e.g., `<think>...</think><answer>...</answer>`). This example uses the GSM8K dataset.
...
1.  **Environment**: Ensure your Python environment has `eval-protocol` and its development dependencies installed:
    ```bash
...
1.  Activate your virtual environment:
    ...
    ```
2.  Execute the `eval-protocol run` command from the root of the repository:
    ```bash
    eval-protocol run --config-path examples/math_with_formatting/conf --config-name run_math_with_formatting_eval
    ```
...
*   **Limit samples**:
    ```bash
    eval-protocol run --config-path examples/math_with_formatting/conf --config-name run_math_with_formatting_eval evaluation_params.limit_samples=5
    ```
    (The default in the example config is `limit_samples: 2`).
*   **Change generation model**:
    ```bash
    eval-protocol run --config-path examples/math_with_formatting/conf --config-name run_math_with_formatting_eval generation.model_name="accounts/fireworks/models/mixtral-8x7b-instruct"
    ```
