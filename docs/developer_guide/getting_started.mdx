...
For evaluating datasets or running complete examples, the primary method is the `eval-protocol run` CLI command. This uses [Hydra for configuration](./hydra_configuration.mdx), allowing you to define your dataset, model, and reward logic in YAML files.
