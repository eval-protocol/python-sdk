...
3.  **Local Evaluation (using `eval-protocol run`)**: Running evaluations locally using Hydra-based configurations to generate responses and score them.
4.  **Previewing Results (using `eval-protocol preview`)**: Inspecting or re-evaluating generated outputs.
...
## 3. Local Evaluation with `eval-protocol run`
...
### Using the CLI (`eval-protocol deploy`)

The `eval-protocol deploy` command is suitable for deploying reward functions defined in script files. The `--metrics-folders` argument should point to the directory containing your reward function script (e.g., a `main.py` with the `@reward_function` decorator).
...
eval-protocol deploy \
  ...
  --force
```
For more details on `eval-protocol deploy`, see the [CLI Reference](../cli_reference/cli_overview.mdx).
...
1. **Iterative Development**: Start simple, test thoroughly, and refine your reward function. Use `eval-protocol run` and `eval-protocol preview` extensively.
