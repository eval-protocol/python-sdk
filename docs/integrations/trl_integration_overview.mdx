...
This directory contains examples showing how to use Eval Protocol reward functions with Hugging Face's Transformer Reinforcement Learning (TRL) library.
...
These examples demonstrate how to integrate Eval Protocol's reward functions with TRL for model fine-tuning. This document serves as the primary guide for the scripts found in the `examples/trl_integration/` directory.
...
*   **`grpo_example.py`**: Demonstrates using reward functions with the Group Relative Policy Optimization (GRPO) trainer from TRL. This is a key example showing:
    *   Creating reward functions compatible with Eval Protocol.
...
- Creating format and accuracy reward functions compatible with Eval Protocol
...
## Integration with Eval Protocol's RewardFunction Class

Eval Protocol's RewardFunction class includes a `get_trl_adapter()` method that converts any reward function into the format expected by TRL. This makes it easy to use existing reward functions from Eval Protocol with TRL trainers.
